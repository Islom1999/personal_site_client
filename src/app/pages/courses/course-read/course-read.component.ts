...
import { ISpCourses, ISpCoursesModule, ISpCoursesModulePart } from '../../../../shared/models/sp-courses.model';
...
export class CourseReadComponent implements OnInit {
  ...
  get courseProgress(): number {
    ...
    const totalParts = this.courseData.sp_courses_modules.reduce(
      (total, module) => total + (module.sp_courses_module_parts?.length || 0), 0
    );
  ...
  selectPart(moduleIndex: number, partIndex: number) {
    ...
    this.currentPartCompleted = this.currentPart ? 
      this.completedParts.has(this.currentPart.id) : false;
  ...
  hasNextPart(): boolean {
    ...
    const isLastPartInModule = this.currentPartIndex >= (currentModule?.sp_courses_module_parts?.length || 0) - 1;
